...
void Bloomfilter::add(const DataObjectRef &dObj)
{
	if(non_counting != NULL)
	{
		bloomfilter_add(
                                non_counting, 
                                (const char *)dObj->getId(), 
                                DATAOBJECT_ID_LEN);
	}else if(counting != NULL)
	{
		counting_bloomfilter_add(
                                counting, 
                                (const char *) dObj->getId(), 
                                DATAOBJECT_ID_LEN);
	}else{
		HAGGLE_ERR(
                                "Tried to add to bloomfilter which is neither counting or "
                                "non-counting!\n");
	}
}
...
void Bloomfilter::remove(const DataObjectRef &dObj)
{
	...
	{
		counting_bloomfilter_remove(
                                counting, 
                                (const char *) dObj->getId(), 
                                DATAOBJECT_ID_LEN);
	}else{
		HAGGLE_ERR(
                                "Tried to remove from bloomfilter which is neither counting or "
                                "non-counting!\n");
	}
}
...
bool Bloomfilter::has(const DataObjectRef &dObj)
{
	...
	{
		return 
                                counting_bloomfilter_check(
                                                counting, 
                                                (const char *) dObj->getId(), 
                                                DATAOBJECT_ID_LEN) != 0;
	}else{
		HAGGLE_ERR(
                                "Tried to check bloomfilter which is neither counting or "
                                "non-counting!\n");
	}
	return false;
}
...
string Bloomfilter::toBase64(void)
{
	...
	}else if(counting != NULL)
	{
		...
		HAGGLE_ERR(
                                "Tried to convert bloomfilter which is neither counting or "
                                "non-counting!\n");
	}
	return retval;
}
...
void Bloomfilter::fromBase64(const string &b64)
{
	...
	}else if(counting != NULL)
	{
		...
		HAGGLE_ERR(
                                "Tried to assign bloomfilter which is neither counting or "
                                "non-counting!\n");
	}
}
...
unsigned long Bloomfilter::countDataObjects(void)
{
	...
	}else if(counting != NULL)
	{
		...
		HAGGLE_ERR(
                                "Tried to get the number of data objects in bloomfilter which is "
                                "neither counting or non-counting!\n");
	}
	return 0;
}
...
const char *Bloomfilter::getRaw(void)
{
	...
	}else if(counting != NULL)
	{
		...
		HAGGLE_ERR(
                                "Tried to get raw bloomfilter which is neither counting or "
                                "non-counting!\n");
	}
	return NULL;
}
...
unsigned long Bloomfilter::getRawLen(void)
{
	...
	}else if(counting != NULL)
	{
		...
		HAGGLE_ERR(
                                "Tried to get length of bloomfilter which is neither counting or "
                                "non-counting!\n");
	}
	return 0;
}
...
void Bloomfilter::setRaw(const char *bf)
{
	if(non_counting != NULL)
	{
		if(	BLOOMFILTER_TOT_LEN(non_counting) != 
			BLOOMFILTER_TOT_LEN((struct bloomfilter *)bf))
		{
	...
	{
		if(	COUNTING_BLOOMFILTER_TOT_LEN(counting) != 
			COUNTING_BLOOMFILTER_TOT_LEN((struct bloomfilter *)bf))
		{
			HAGGLE_ERR("Old and new bloomfilter differ in length!\n");
		}else{
		...
		HAGGLE_ERR(
                                "Tried to set bloomfilter which is neither counting or "
                                "non-counting!\n");
	}
}
...
void Bloomfilter::reset(void)
{
	...
	}else if(counting != NULL)
	{
		...
		HAGGLE_ERR(
                                "Tried to reset bloomfilter which is neither counting or "
                                "non-counting!\n");
	}
}
